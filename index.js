var jsdom = require('jsdom'),
  ...
  clint.command('--aggressive', '-a', 'Remove whitespaces inside template (-a includes -m)')
  ...
  parseHTML(html, relativePath, function() {
    var output = []
      ...
      function inlineScript(array) {
        return array.map(function(entry) {
          var script = entry
          if (entry.indexOf('!function') != 0) script = fs.readFileSync(entry) + ';'
          if (!options.mangle) return script
          return escodegen.generate(esmangle.mangle(esprima.parse(script)), {
            format: {
              renumber: true,
              hexadecimal: true,
              escapeless: true,
